...
internal fun browseDiscoveryFlow(
    ...
    fun newListener(): NsdManager.DiscoveryListener = object : NsdManager.DiscoveryListener {
        ...
        override fun onServiceFound(service: NsdServiceInfo) {
            if (service.serviceType != serviceType && !service.serviceType.contains("squadbots")) {
                return
            }
            attemptResolve(service, attemptsSoFar = 1)
        }
...
/**
 * NsdManager wrapper for `_squadbots._tcp`, exposed as a Flow of [DiscoveryState].
 */
class NsdDiscovery(
    context: Context,
    ...
    companion object {
        const val SERVICE_TYPE = "_squadbots._tcp."
        const val MULTICAST_LOCK_TAG = "squadbots-nsd"
    }
}
