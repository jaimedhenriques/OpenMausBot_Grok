...
describe("canonical thread links", () => {
  ...
  it("copies and parses one spelling, and rejects near-misses", () => {
    ...
    expect(link).toBe(`squadbots://thread/${uuid}?bot=scout`);
    // the paste path accepts exactly what copy emits
    expect(parseThreadRefUrl(link)).toEqual({ threadId: uuid, botId: "scout" });
    expect(parseThreadRefUrl(`squadbots://thread/${uuid}`)).toEqual({ threadId: uuid });
    for (const miss of [
      `squadbots://thread/${uuid}/extra?bot=scout`,
      `squadbots://thread/${uuid}?bot=scout&x=1`,
      `squadbots://thread/${uuid}?bot=`,
  ...
  it("turns a pasted link or raw UUID into the title token, and leaves unknown ids plain", () => {
    ...
    expect(threadTokenFromPaste("squadbots://thread/" + crypto.randomUUID(), uuidThreads)).toBeNull();
  ...
  it("sends resolvable titles as canonical markdown and passes everything else through", () => {
    ...
    const sent = `Already [Release notes](${link}) and [Gone](squadbots://thread/dead?bot=ada)`;
    ...
    expect(serializeThreadRefs("see #QA [PR] 245", bracketed)).toBe("see [QA \\[PR\\] 245](squadbots://thread/b?bot=scout)");
  });
  ...
  it("displays canonical links as title chips and dead links as raw text", () => {
    ...
    expect(splitThreadRefsForDisplay("[Gone](squadbots://thread/dead?bot=ada)", threads))
      .toEqual([{ text: "[Gone](squadbots://thread/dead?bot=ada)" }]);
  });
