...
describe("canonical thread links", () => {
  ...
  it("copies and parses one spelling, and rejects near-misses", () => {
    ...
    expect(link).toBe(`softbots://thread/${uuid}?bot=scout`);
    // the paste path accepts exactly what copy emits
    expect(parseThreadRefUrl(link)).toEqual({ threadId: uuid, botId: "scout" });
    expect(parseThreadRefUrl(`softbots://thread/${uuid}`)).toEqual({ threadId: uuid });
    for (const miss of [
      `softbots://thread/${uuid}/extra?bot=scout`,
      `softbots://thread/${uuid}?bot=scout&x=1`,
      `softbots://thread/${uuid}?bot=`,
  ...
  it("turns a pasted link or raw UUID into the title token, and leaves unknown ids plain", () => {
    ...
    expect(threadTokenFromPaste("softbots://thread/" + crypto.randomUUID(), uuidThreads)).toBeNull();
  ...
  it("sends resolvable titles as canonical markdown and passes everything else through", () => {
    ...
    const sent = `Already [Release notes](${link}) and [Gone](softbots://thread/dead?bot=ada)`;
    ...
    expect(serializeThreadRefs("see #QA [PR] 245", bracketed)).toBe("see [QA \\[PR\\] 245](softbots://thread/b?bot=scout)");
  });
  ...
  it("displays canonical links as title chips and dead links as raw text", () => {
    ...
    expect(splitThreadRefsForDisplay("[Gone](softbots://thread/dead?bot=ada)", threads))
      .toEqual([{ text: "[Gone](softbots://thread/dead?bot=ada)" }]);
  });
