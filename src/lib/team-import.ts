...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "squadbots.backup") {
  ...
  if (root.format === "squadbots.package") return packagePreview(root, manifest);
  if (root.format !== "squadbots.team") throw new Error("This is not an Squadbots backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "squadbots.package", version: 1, package: pkg };
}
