...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "softbots.backup") {
  ...
  if (root.format === "softbots.package") return packagePreview(root, manifest);
  if (root.format !== "softbots.team") throw new Error("This is not an Squadbots backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "softbots.package", version: 1, package: pkg };
}
