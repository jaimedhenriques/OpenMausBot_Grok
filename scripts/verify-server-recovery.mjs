...
if (process.versions.electron && process.argv.includes(flag)) {
  ...
  app.whenReady().then(async () => {
    ...
    const leaseFile = join(dataDir, "squadbots-server.lease");
    ...
    const start = async () => {
      ...
      proc.once("spawn", () => {
        ...
        proc.postMessage({ type: "squadbots:desktop-mutation-token", token, companionToken: token });
      });
    ...
    if (scenario === "recover") {
      ...
      assert.equal(JSON.parse(readFileSync(join(dataDir, ".softbots-server-child/squadbots-server.lease"), "utf8")).pid, replacementPid);
