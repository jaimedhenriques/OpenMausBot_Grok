#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Softbots
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Softbots URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Softbots URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Softbots URL must not contain credentials; use SOFTBOTS_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("Softbots URL must be an origin without a path, query, or fragment");
  }
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !process.env.SOFTBOTS_TOKEN?.trim()) {
      throw new Error(
        "Softbots refused this write because the installed desktop app requires a paired session token. " +
        "Set SOFTBOTS_TOKEN as described in docs/mcp-server.md.",
      );
    }
    throw new Error(`Softbots API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`Softbots API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "softbots") {
        failures.push(`${candidate} answered, but it was not Softbots`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running Softbots server. ${failures.join("; ")}`);
}
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Softbots server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Softbots ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "softbots") throw new Error("The configured endpoint is not an Softbots server");
      return {
        status: "connected",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Softbots did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Softbots did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Softbots did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Softbots did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Softbots did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Softbots did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("Softbots did not return the updated task");
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the Softbots team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Softbots MCP server running on stdio");
}
