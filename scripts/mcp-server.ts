#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Squadbots
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Squadbots URL: '${url}'`);
  }
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  try {
    ...
    throw new Error(`Squadbots API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  throw new Error(`Could not find a running Squadbots server. ${failures.join("; ")}`);
}
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Squadbots server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Squadbots ID`);
  return value;
}
  ...
  switch (name) {
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Squadbots did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Squadbots did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Squadbots did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Squadbots did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Squadbots did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Squadbots did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("Squadbots did not return the updated task");
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Squadbots MCP server running on stdio");
}
