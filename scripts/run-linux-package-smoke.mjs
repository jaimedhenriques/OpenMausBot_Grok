...
const executables = [
  path.join(root, "release", "linux-unpacked", "squadbots"),
  path.join(root, "release", appImage),
];
if (process.env.OMB_SMOKE_INSTALLED_DEB === "1") {
  executables.push("/opt/Squadbots/squadbots");
}
  ...
  const hardDeath = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        OMB_SMOKE_EXECUTABLE: path.join(root, "release", "linux-unpacked", "squadbots"),
      },
      stdio: "inherit",
    },
  );
