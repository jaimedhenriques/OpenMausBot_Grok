// Assemble the `softbots` npm package: the self-contained server bundle,
// the built UI, the bundled skills and the CLI, with a package.json of its
// own. `npx softbots serve` then needs Node 24+ and nothing else.
//
...
Or run \`npx softbots\` without a global install. Use the same command next
time; \`softbots start\` is an alias for the bare command.
...
\`\`\`sh
softbots setup          # reconfigure AI and optional phone access; not a reset
softbots --no-open      # do not open a browser
softbots --local        # ignore saved remote access for this launch
softbots --no-pair      # suppress phone prompts and invitations
softbots pair           # another phone while the HTTPS workspace is running
softbots sessions       # list devices; sessions revoke ID signs one out
softbots serve          # no onboarding prompts; explicit remote flags for services
\`\`\`
...
\`softbots setup\` to continue without deleting your data.
