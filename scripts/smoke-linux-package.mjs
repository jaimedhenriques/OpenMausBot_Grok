...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "softbots"),
);
...
for (const appName of ["softbots", "Squadbots", "Squadbots"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "ai.squadbots.app",
};
...
try {
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["softbots", "Squadbots", "Squadbots"]
    ...
    const userData = ["softbots", "Squadbots", "Squadbots"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
