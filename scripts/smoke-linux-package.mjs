...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "squadbots"),
);
...
mkdirSync(path.join(home, ".softbots"), { recursive: true });
...
for (const appName of ["squadbots", "Squadbots", "Squadbots"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "ai.softbots.app",
};
...
try {
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["squadbots", "Squadbots", "Squadbots"]
    ...
    const userData = ["squadbots", "Squadbots", "Squadbots"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
