...
/** Pairing tokens may travel through a verified Squadbots HTTPS endpoint or
...
export function normalizeDesktopCompanionEndpoint(value) {
  ...
  const managedHttps =
    parsed.protocol === "https:" && hostname.endsWith(".softbots.com");
  if (
...
export async function pairDesktopCompanion({
  ...
  if (!endpoint) {
    throw new Error("Enter the Squadbots HTTPS companion address or full Tailscale name ending in .ts.net");
  }
