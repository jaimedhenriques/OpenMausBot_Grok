...
test("utility replies cannot be forged by another child or reused across requests", async () => {
  ...
  relay.receive(foreign, { type: "squadbots:managed-desktop-result", requestId: message.requestId, ok: true });
  await settle(); assert.equal(completed, false);
  relay.receive(proc, { type: "squadbots:managed-desktop-result", requestId: message.requestId, ok: true });
