...
Squadbots saves a new server switched off. Use **Test** to start the command
...
`OMB_CLAUDE_INHERIT_USER_CONFIG=1` on the Squadbots process remains the full
...
window with `--autocompact` (2.1.122+). Squadbots reads `claude --version`
...
The same registry lives in `~/.softbots/config.json`:
...
by hand, restart Squadbots. Every bot whose engine can mount custom MCP
...
- **Testing is bounded.** A command is stopped after the handshake (or eight
  ...
  inherits none of Squadbots's workspace or provider credentials; only the
