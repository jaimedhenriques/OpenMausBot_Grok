# Self-hosting the Softbots server
...
Step by step, for a server you do not have yet: [Deploy Softbots on a
...
- a browser for bots, once the engine is installed on the server
  (`npx softbots browser install`, or nothing to do in the Docker image,
...
and creates no pairing invitation. Use `npx softbots setup` to configure without starting, or
`npx softbots serve` to start non-interactively with your existing config
...
`npx softbots browser install` as the service user so that user's browser
is present. Three ways to make the server reachable from elsewhere:

- **On your Tailscale network, no domain needed:**
  `npx softbots serve --tailscale`. Tailscale terminates HTTPS with its
...
- **A public address, no domain, no proxy, no open port:**
  ...
  ```sh
  npx softbots login          # once: an emailed code signs this machine in
  npx softbots serve --tunnel
  ```
  ...
  the server treats as "through a proxy", never as the owner. `npx softbots
...
- **Your own domain, still one command:**
  ...
  ```sh
  npx softbots serve --domain maus.example.com
  ```
...
- **Behind your own proxy or domain:** `npx softbots serve --public-url
  https://maus.example.com`, with the proxy rules from "Putting a proxy in
  front".

Later: `npx softbots pair --label "Kitchen iPad"` for another device
(`--client` for one that may chat but not change settings), and
`npx softbots sessions` to see or revoke them. `softbots serve` is a
...
(`ghcr.io/milind-soni/softbots`, tagged `latest`, `sha-…` and `v…`).
...
docker compose exec omb node dist-server/softbots.js pair # prints a code, a link and a QR
```
...
```sh
npx softbots service install --domain maus.example.com   # or --tunnel, --tailscale, or nothing
```

It renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
same `softbots serve …` with your options, restarts it if it stops, and,
...
`softbots service uninstall` prints the reverse. Install the package
permanently first (`npm install -g softbots`): a service must not point
...
`softbots fleet` runs one workspace per client on a single Linux server,
each as its own OS user, its own `softbots@<name>` service on its own
...
```sh
softbots fleet init --domain example.com
```
...
```sh
softbots fleet create acme --admin owner@acme.test --member @acme.test \
  --brand /root/acme-brand.json --anthropic-key-file /root/acme-anthropic.key \
  --cap 50 --memory 1G
softbots fleet users acme add bob@acme.test --chat-only
softbots fleet list
softbots fleet suspend acme      # 503 page, service stopped; resume undoes it
softbots fleet upgrade           # new release, then every running workspace restarted in turn
softbots fleet delete acme --yes # add --keep-data to keep the home folder
```
...
agent's audit log at `/var/log/softbots/fleet.jsonl`.
...
```sh
npx softbots pair                         # npm install
pnpm omb pair                                # from a checkout
docker compose exec omb node dist-server/softbots.js pair   # Docker
```
...
link for each device: `npx softbots pair --label "My desktop"` creates an
...
`GET /.well-known/softbots/environment` is public and tells a client what
...
```sh
npx softbots access add her@yourcompany.com
npx softbots access add freelancer@example.com --chat-only
npx softbots access list
```

An entry is an address or `@domain` (everyone at that domain). Admins get
the same access as a pairing code from `softbots serve`; members get the
chat-only scope, the same as `softbots pair --client`. The same lists live
...
The code itself comes from `accounts.softbots.com`, the Softbots
...
`softbots sessions` and can be revoked the same way.
...
code that `softbots serve` (or `softbots pair`) prints, paste the
...
its own, listed and revocable with `softbots sessions`. What it may do is
the code's scope: a code from `softbots pair` carries `admin` and the app
shows everything; a code from `softbots pair --client` (also what the
...
Both native apps pair this way. `softbots pair --phone android` prints the
...
terminal, such as `docker compose exec omb node dist-server/softbots.js
...
credentials, which are **not** `softbots sessions` and are revoked from
...
For a foreground invocation, `npx --yes softbots@X.Y.Z serve --tunnel`
...
git pull && pnpm install && sudo systemctl restart softbots          # from source
```
