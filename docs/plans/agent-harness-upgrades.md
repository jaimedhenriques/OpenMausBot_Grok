# Squadbots harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what Squadbots lacked by
...
**The reframe that reorders everything.** Squadbots is itself a harness. The
...
**On-disk forward compatibility.** `~/.softbots` is shared across app versions.
