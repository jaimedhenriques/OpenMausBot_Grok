...
```sh
squadbots fleet init --domain example.test --dry-run
squadbots fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g squadbots`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `squadbots fleet init --domain <domain>`; check `systemctl status
   squadbots-fence` and `nft list table inet squadbots`.
3. `squadbots fleet create alpha --admin you@example.test --cap 1` and the
...
4. Isolation: as `omb-alpha` (`runuser -u omb-alpha -- curl -s
   ...
   as root it must answer. `ls /var/lib/squadbots/beta` as `omb-alpha` must
   be denied.
