...
```sh
softbots fleet init --domain example.test --dry-run
softbots fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g softbots`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `softbots fleet init --domain <domain>`; check `systemctl status
   softbots-fence` and `nft list table inet softbots`.
3. `softbots fleet create alpha --admin you@example.test --cap 1` and the
...
4. Isolation: as `omb-alpha` (`runuser -u omb-alpha -- curl -s
   ...
   as root it must answer. `ls /var/lib/softbots/beta` as `omb-alpha` must
   be denied.
