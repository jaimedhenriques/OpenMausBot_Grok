...
The optional functional test launches the real Squadbots server, its OpenCode
...
The checks require Squadbots to retain this catalog metadata, advertise its
...
Squadbots server with the same disposable data directory.
...
provider, an upstream gateway's routing, or the Squadbots GUI. The fixture
