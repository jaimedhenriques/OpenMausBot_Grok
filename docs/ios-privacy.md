# SoftbotsMobile privacy

SoftbotsMobile is a companion for an Softbots service chosen and operated
by the user. Local Wi-Fi and Tailscale connections work without an Softbots
...
- The computer remains the source of bots, transcripts, approvals, credentials,
  SQLite data, and screen images. Softbots's hosted control plane does not
...
- When a user completes a supported credential request on the phone, the value
  ...
  written to
  iOS preferences or Keychain by SoftbotsMobile, chat, SQLite, logs, or the
...
- If the desktop user enables optional hosted access, Softbots stores the
...
- The optional HTTPS route is proxied by Cloudflare to an outbound-only
  ...
  but are not written to the Softbots control-plane database. Cloudflare may
  process IP addresses and connection/request metadata as Softbots's service
...
- Connector tokens stay in the desktop operating system's encrypted credential
  ...
  database.
- When a user explicitly chooses Softbots from another app's Share sheet, or
  ...
  Share sheet session removes the abandoned copy immediately; an SoftbotsMobile
...
Revoking the phone in Softbots's Companion settings invalidates that device
credential. Transcript deletion is controlled by the Softbots installation
...
[Softbots Support](https://github.com/milind-soni/OpenMausBot/issues) request
...
[Softbots Support](https://github.com/milind-soni/OpenMausBot/issues).
