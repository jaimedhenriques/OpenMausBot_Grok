# SquadbotsMobile privacy

SquadbotsMobile is a companion for an Squadbots service chosen and operated
by the user. Local Wi-Fi and Tailscale connections work without an Squadbots
...
- The computer remains the source of bots, transcripts, approvals, credentials,
  SQLite data, and screen images. Squadbots's hosted control plane does not
...
- When a user completes a supported credential request on the phone, the value
  ...
  written to
  iOS preferences or Keychain by SquadbotsMobile, chat, SQLite, logs, or the
...
- If the desktop user enables optional hosted access, Squadbots stores the
...
- The optional HTTPS route is proxied by Cloudflare to an outbound-only
  ...
  but are not written to the Squadbots control-plane database. Cloudflare may
  process IP addresses and connection/request metadata as Squadbots's service
...
- Connector tokens stay in the desktop operating system's encrypted credential
  ...
  database.
- When a user explicitly chooses Squadbots from another app's Share sheet, or
  ...
  Share sheet session removes the abandoned copy immediately; an SquadbotsMobile
...
Revoking the phone in Squadbots's Companion settings invalidates that device
credential. Transcript deletion is controlled by the Squadbots installation
...
[Squadbots Support](https://github.com/jaimedhenriques/OpenMausBot_Grok/issues) request
...
[Squadbots Support](https://github.com/jaimedhenriques/OpenMausBot_Grok/issues).
