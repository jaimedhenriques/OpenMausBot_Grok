...
[Squadbots releases](https://github.com/jaimedhenriques/OpenMausBot_Grok/releases).
...
0.1.46 point to `milind-soni/squadbots-releases`; newer builds point to
`jaimedhenriques/OpenMausBot_Grok`. For that reason:

1. Every new release is published byte-for-byte to both repositories during
   the bridge period.
2. `squadbots-releases` must stay public. Do not delete its final bridge
...
Set these in **Squadbots → Settings → Secrets and variables → Actions**.
...
Fine-grained tokens** → repository access: only `squadbots-releases` →
