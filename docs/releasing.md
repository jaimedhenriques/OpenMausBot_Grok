...
1. Every new release is published byte-for-byte to both repositories during
   the bridge period.
2. `softbots-releases` must stay public. Do not delete its final bridge
...
Set these in **Softbots → Settings → Secrets and variables → Actions**.
...
Fine-grained tokens** → repository access: only `softbots-releases` →
