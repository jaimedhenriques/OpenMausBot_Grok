# Deploy Squadbots on a VPS

From a blank Linux server to Squadbots running on it around the clock, reachable from your laptop, the desktop app and your phone, with your bots working while every laptop is closed. It assumes nothing beyond being able to open a terminal and paste commands. About twenty minutes, most of it waiting.
...
|---|---|---|---|
| **A. Public address, no domain** (`serve --tunnel`) | an Squadbots account (email code) | anyone with a pairing code, over HTTPS | the fastest path; a phone on cellular |
...
```sh
npx squadbots setup          # once: choose AI access, connect, and choose a model
npx squadbots login          # once: an emailed code signs this machine in and reserves its address
npx squadbots serve --tunnel # runs the server there and prints the pairing link with a QR code
```

`setup` connects an AI provider; it is separate from the Squadbots account.
...
The account credentials live in `~/.softbots/tunnel-account.json`, readable only by your user. `npx squadbots logout` releases the address.
...
3. **Get the deploy files and set the name:**
   ...
   ```sh
   git clone https://github.com/milind-soni/OpenMausBot && cd Squadbots/deploy
   cp .env.example .env
...
In this path, every `npx squadbots …` command below is run inside the container instead:

```sh
docker compose exec omb node dist-server/squadbots.js pair --label "My MacBook"
```
...
```sh
sudo -H npx --yes squadbots browser install --with-deps
```

Then install the browser in the service account's home, from the `maus` shell:

```sh
npx --yes squadbots browser install
```
...
The npm Squadbots package does not install model engine CLIs. For paths A and C,
...
`~/.claude`), separately from Squadbots's `~/.softbots`. Keep that home when
...
```sh
npx squadbots pair --label "Kitchen iPad"
```
...
```sh
npx squadbots sessions              # id, device, scope, last seen, expires
npx squadbots sessions revoke ID    # signs that device out and closes its stream at once
npx squadbots status                # what the server says about itself
```

## Keep it running

`npx squadbots serve` is a plain foreground process. For systemd (paths A and C),
install a chosen release first, from the `maus` shell. Replace `X.Y.Z` with the
published version you want to run:

```sh
npm install --global --prefix "$HOME/.local" squadbots@X.Y.Z
```

Stop the foreground server with Ctrl-C before enabling the service. From the
administrator shell, save this as `/etc/systemd/system/squadbots.service`:

```ini
# /etc/systemd/system/squadbots.service
[Unit]
Description=Squadbots server
...
ExecStart=/home/maus/.local/bin/squadbots serve --tunnel --no-pair
Restart=always
RestartSec=5
...
```sh
sudo systemctl daemon-reload && sudo systemctl enable --now squadbots
journalctl -u squadbots -f            # the server's log, including "tunnel: live at …"
```

Use `--tailscale` instead of `--tunnel` for path C. `--no-pair` skips printing a code at every restart; mint one with `npx squadbots pair` when you need it. Docker (path B) restarts on its own (`restart: unless-stopped`).
...
- **Paths A and C with the service above:** choose the new published version,
  ...
  ```sh
  sudo systemctl stop squadbots
  sudo -iu maus npm install --global --prefix /home/maus/.local squadbots@X.Y.Z
  sudo systemctl start squadbots
  ```
  ...
  desired release as `npx --yes squadbots@X.Y.Z serve --tunnel`.
- **Path B:** `cd Squadbots/deploy && docker compose pull omb && docker compose up -d`.
...
process, or `sudo systemctl stop squadbots` from the administrator shell for
...
```sh
umask 077
backup_dir=$(mktemp -d "$PWD/squadbots-backup.XXXXXX")
tar czf "$backup_dir/squadbots-data.tgz" -C "$HOME" .softbots
```
...
`Squadbots/deploy`, stop the app before archiving; `deploy_data` is the default
volume name, so use your actual volume name if you changed the Compose project:

```sh
docker compose stop omb
backup_dir=$(mktemp -d "$PWD/squadbots-backup.XXXXXX")
docker run --rm -v deploy_data:/data:ro -v "$backup_dir":/b alpine sh -c 'umask 077; tar czf /b/squadbots-data.tgz -C /data .'
```
...
service with `sudo systemctl start squadbots`, or `docker compose start omb`.
...
- The one thing a stranger can read is `/.well-known/squadbots/environment` (the server's id, label, version, capabilities) and `/api/health` (only the app name). Everything else answers "pair this device".

## Troubleshooting

**`serve --tunnel` says "no account on this machine yet".** Run `npx squadbots login` on this machine first; the credentials are per machine.

**The tunnel stays on "retrying".** The server is running and usable locally; the public hop is not verified yet. Wait a minute (Cloudflare needs a moment on a fresh address), then check `journalctl`/the terminal for the reason. If it never comes up, `npx squadbots logout && npx squadbots login` issues a fresh address.
...
**What does the server think it is?** `https://<address>/.well-known/squadbots/environment` is public and shows its id, label, version and capabilities; `npx squadbots status` prints the same on the server.

**Something else.** `journalctl -u squadbots --since -10m` (or `docker compose logs omb --tail 100`) shows the server's startup lines. Paste them with your question in the community channel.
...
```sh
sudo install -d -o root -g root -m 0755 /opt/squadbots-browser
sudo cp -R /home/maus/.agent-browser/browsers/chrome-VERSION /opt/squadbots-browser/
sudo chown -R root:root /opt/squadbots-browser/chrome-VERSION
sudo chmod -R go-w /opt/squadbots-browser/chrome-VERSION
```

Save this as the root-owned `/etc/apparmor.d/squadbots-chrome`, replacing
...
profile squadbots-chrome /opt/squadbots-browser/chrome-VERSION/chrome flags=(unconfined) {
  userns,
}
```
...
```sh
sudo apparmor_parser -r /etc/apparmor.d/squadbots-chrome
```
...
`sudo systemctl restart squadbots`:

```ini
Environment=AGENT_BROWSER_EXECUTABLE_PATH=/opt/squadbots-browser/chrome-VERSION/chrome
```
