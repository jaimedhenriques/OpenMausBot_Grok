...
provider's own permission modes, passed through. Softbots does not judge an
...
| **Custom (`config.toml`)** | Codex only. Softbots reads and reapplies the effective approval and sandbox settings from your Codex configuration. |
...
ends. Softbots keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; Softbots does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

Softbots's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; Softbots keeps its
