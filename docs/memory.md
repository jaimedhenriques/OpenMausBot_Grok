...
folder on the computer running Squadbots — nothing is stored anywhere else,
...
disk, those buttons only work from the computer running Squadbots — a
...
`~/.softbots/memory-journal/<botId>.ndjson`. It lives *outside* the
