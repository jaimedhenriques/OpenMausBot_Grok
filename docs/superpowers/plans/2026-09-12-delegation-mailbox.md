...
- Work only in the worktree `~/Desktop/softbots/OpenGrokBot-mailbox`, on branch `feat/delegation-mailbox`. Never touch `~/Desktop/softbots/OpenGrokBot`.
...
```
"List the other bots (agents) in your Softbots section, with their model and whether they're busy.
```

to:

```
"List the other bots (agents) in your Softbots section, with their model and what each is doing right now (available, working, waiting on the user, not responding, or unavailable).
```
