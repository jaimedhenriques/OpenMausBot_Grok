...
The iOS app is a thin, native client for the Softbots instance running on
...
an Softbots account of its own.
...
The Mac must be running Softbots and must not be asleep. Desktop
...
to reach. Softbots then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use an Softbots-operated
...
The Softbots control plane stores account and installation metadata plus
...
The Mac must remain awake with Softbots running for chats, approvals, and
...
An Softbots account is not required for nearby, manual, or Tailscale
...
Passwords or any enabled third-party AutoFill provider. SoftbotsMobile does
