...
The iOS app is a thin, native client for the Squadbots instance running on
...
an Squadbots account of its own.
...
The Mac must be running Squadbots and must not be asleep. Desktop
...
The sidecar keeps its device registry in `~/.softbots/devices.json`. That is
...
to reach. Squadbots then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use an Squadbots-operated
...
The Squadbots control plane stores account and installation metadata plus
...
The Mac must remain awake with Squadbots running for chats, approvals, and
...
An Squadbots account is not required for nearby, manual, or Tailscale
...
Passwords or any enabled third-party AutoFill provider. SquadbotsMobile does
