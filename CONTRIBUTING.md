...
[Squadbots releases](https://github.com/jaimedhenriques/OpenMausBot_Grok/releases). Then verify the checksum
...
Data lives in `~/.softbots/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.softbots`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.softbots/config.json` via `PUT /api/config` and the API
