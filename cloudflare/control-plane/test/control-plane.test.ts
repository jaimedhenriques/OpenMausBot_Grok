...
const BASE_URL = "https://auth.softbots.test";
...
describe("HTTP boundary hardening", () => {
  ...
  it("defaults CORS to deny and never emits a wildcard", async () => {
    ...
    const allowed = await call("/v1/me", { origin: "https://app.softbots.test" });
    expect(allowed.status).toBe(401);
    expect(allowed.headers.get("access-control-allow-origin")).toBe("https://app.softbots.test");
    expect(allowed.headers.get("cache-control")).toBe("no-store");

    const deniedPreflight = await call("/v1/installations", {
      method: "OPTIONS",
      origin: "https://app.softbots.test",
      headers: {
    ...
    expect(deniedPreflight.headers.get("access-control-allow-origin")).toBe("https://app.softbots.test");
  });
});
