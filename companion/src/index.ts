...
parentPort?.on("message", ({ data }) => {
  ...
  if (message.type !== "squadbots:companion-mutation-token") return;
...
const SERVICE_TYPE = "_squadbots._tcp";
...
const machineName = (): string => cachedName || "Squadbots";
...
async function refreshMachineName(): Promise<void> {
  ...
  try {
    ...
    /* not up, or no profile — "Squadbots" is a fine thing to be called */
  }
}
