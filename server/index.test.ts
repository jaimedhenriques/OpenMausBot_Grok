...
const managedBoxNameForFixture = (botId: string): string => {
  ...
  const environmentId = readFileSync(join(home, ".softbots", "environment-id"), "utf8").trim();
...
const storedMessageCount = (threadId: string): number => {
  const db = new DatabaseSync(join(home, ".softbots", "messages.db"), { readOnly: true });
  try {
...
beforeAll(async () => {
  ...
  fakeDockerFixture = join(home, ".softbots", "fake-unmanaged-container");
  fakeVpsFixture = join(home, ".softbots", "fake-vps-container.json");
  fakeDockerLog = join(home, ".softbots", "fake-docker-calls.log");
  ...
  writeFileSync(fakeDockerProgram, [
    ...
    '    process.stdout.write(JSON.stringify([{ Id: spec.id, Name: `/${spec.name}`, Config: { Labels: { "com.softbots.vps": "1", "com.softbots.container": spec.name } }, State: { Status: "exited", Running: false } }]));',
    '    process.exit(0);',
    '  }',
    ...
    '    const labels = spec?.managed ? { "com.softbots.local-vm": "1", "com.softbots.workspace": "1", "com.softbots.local-vm-target": spec.targetLabel } : {};',
  ...
  writeFileSync(
    join(home, ".softbots", "groups.json"),
    JSON.stringify([
      {
        id: "test-dm",
  ...
  const linkedWorkspace = join(home, ".softbots", "workspaces", "test-bot-a");
  const linkedFile = join(linkedWorkspace, "phone report.md");
  const linkedImage = join(linkedWorkspace, "preview.png");
  const privateAttachments = join(home, ".softbots", "attachments");
  ...
  writeFileSync(
    join(home, ".softbots", "messages-test-linked-file-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".softbots", "messages-test-goal-restart-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".softbots", "routines.json"),
    JSON.stringify({
      version: 1,
      routines: [],
      runs: [{
  ...
  writeFileSync(
    join(home, ".softbots", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".softbots", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
...
describe("harness HTTP API", () => {
  ...
  it("refuses a second live server that shares the same data directory", async () => {
    ...
    const contender = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      ...
      env: {
        ...(process.env.PATH ? { PATH: process.env.PATH } : {}),
        ...(process.env.SystemRoot ? { SystemRoot: process.env.SystemRoot } : {}),
        OMB_DATA_DIR: join(home, ".softbots"),
  ...
  it.each([
    ...
    const key = join(home, ".softbots", "browser-engine-key");
    ...
    const journal = () => JSON.parse(readFileSync(join(home, ".softbots", "browser-cleanups.json"), "utf8")) as Array<{ id: string; phase: string }>;
    try {
  ...
  it("creates team computers only with consent, retains failed creates, and assigns safely", async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(readFileSync(join(home, ".softbots", "team-computers.json"), "utf8"));
      ...
      const saved = JSON.parse(readFileSync(join(home, ".softbots", "team-computers.json"), "utf8"));
  ...
  it("keeps Full and Custom bots on Codex when the paired model route changes providers", async () => {
    ...
    const isolatedData = join(isolatedHome, ".softbots");
  ...
  it("clears incompatible default and per-agent voices when the provider changes", async () => {
    ...
    try {
      ...
      const disk = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
  ...
  it("does not switch voice providers when per-agent voices cannot be cleared", async () => {
    let botId = "";
    const botsPath = join(home, ".softbots", "bots.json");
  ...
  it("retires a journaled Box proven gone before clearing and later restoring credentials", async () => {
    ...
    try {
      ...
      expect((await api("PUT", "/api/config", { box: { token: "" } })).status).toBe(200);
      const journal = JSON.parse(readFileSync(join(home, ".softbots", "box-create-requests.json"), "utf8"));
  ...
  it("never lets an out-of-band SOUL.md edit reach the prompt, and surfaces it as drift instead", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".softbots", "bots", bot.id, "SOUL.md"), "File text.");
      rmSync(fakeClaudeDump, { force: true });
      expect((await api("POST", `/api/bots/${bot.id}/messages`, { text: "hello" })).status).toBe(202);
      await expect.poll(() => existsSync(fakeClaudeDump), { timeout: 5_000 }).toBe(true);
      const seen = JSON.parse(readFileSync(fakeClaudeDump, "utf8"));
  ...
  it("keeps skill authoring on by default and persists an explicit opt-out", async () => {
    ...
    const untouched = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
  ...
  it("clears bot references when a named browser profile is removed", async () => {
    ...
    try {
      ...
      const config = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
  ...
  it("keeps a proposed profile change inert until its card is confirmed, then records history", async () => {
    const soulFileOf = (botId: string) => join(home, ".softbots", "bots", botId, "SOUL.md");
  ...
  it("only enables the exact learned-skill proposal a current client reviewed", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(home, ".softbots", "skill-state", bot.id, "staged.json"),
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".softbots", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".softbots", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
  ...
  it("keeps avatar providers and externally stored image credentials separate", async () => {
    try {
      ...
      for (const secret of ["openai-avatar-fixture", "custom-avatar-fixture"]) {
        ...
        expect(readFileSync(join(home, ".softbots", "config.json"), "utf8")).not.toContain(secret);
      }
      const disk = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".softbots", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".softbots", "secret.md"), "TOP-SECRET-MARKER sibling");
  ...
  it("round-trips soul through both PATCH routes and mirrors it to SOUL.md", async () => {
    ...
    expect(existsSync(join(home, ".softbots", "bots", bot.id))).toBe(false);
  });
  ...
  it("keeps mixed-request runtime revocations effective when profile persistence fails", async () => {
    ...
    const botsFile = join(home, ".softbots", "bots.json");
  ...
  it("refuses redacted history restores without changing SOUL and still restores exact safe text", async () => {
    ...
    const file = join(home, ".softbots", "bots", bot.id, "history.ndjson");
...
describe("message pages", () => {
  ...
  it("downloads only a file linked by the exact stored bot message", async () => {
    ...
    const linkedFile = join(home, ".softbots", "workspaces", "test-bot-a", "phone report.md");
    const response = await fetch(
    ...
    expect((await api(
      ...
      { path: join(home, ".softbots", "workspaces", "test-bot-a", "other.md") },
    )).status).toBe(403);
  ...
  it("downloads an image rendered by the exact stored bot message", async () => {
    const threadId = "test-linked-file-room-thread";
    const linkedImage = join(home, ".softbots", "workspaces", "test-bot-a", "preview.png");
  ...
  it("downloads an exact user attachment only from the private attachment store", async () => {
    const threadId = "test-linked-file-room-thread";
    const shared = join(home, ".softbots", "attachments", "shared-notes.pdf");
    const response = await fetch(
    ...
    expect((await api(
      ...
      { path: join(home, ".softbots", "attachments", "different.pdf") },
    )).status).toBe(403);
    expect((await api(
      "POST",
      `/api/threads/${threadId}/messages/user-outside-file-message/file`,
      { path: join(home, ".softbots", "workspaces", "test-bot-a", "phone report.md") },
    )).status).toBe(403);
  });
...
describe("instance CLI override API", () => {
  it("round-trips bounded per-instance icons without changing the driver", async () => {
    const before = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
    ...
    const savedPreset = JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8"));
    ...
    expect(JSON.parse(readFileSync(join(home, ".softbots", "config.json"), "utf8")).instances.ghost.icon).toBeUndefined();
  });
