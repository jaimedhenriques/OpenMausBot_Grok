...
describe("chiefOfStaffSystemPrompt", () => {
  ...
  it("includes trusted Squadbots status only when the Chief caller supplies it", () => {
    const status = "TRUSTED SQUADBOTS STATUS\nfreshness=fresh; runtime_state=degraded";
    ...
    expect(ordinaryPrompt).not.toContain("TRUSTED SQUADBOTS STATUS");
  });

  it("pins legacy delegation guidance and shared staffing independently of the bounded chat coordinator", () => {
    const prompt = chiefOfStaffSystemPrompt("chief", bots, true, "TRUSTED SQUADBOTS STATUS\nfreshness=fresh");
