...
describe("chiefOfStaffSystemPrompt", () => {
  ...
  it("includes trusted Softbots status only when the Chief caller supplies it", () => {
    const status = "TRUSTED SOFTBOTS STATUS\nfreshness=fresh; runtime_state=degraded";
    ...
    expect(ordinaryPrompt).not.toContain("TRUSTED SOFTBOTS STATUS");
  });

  it("pins legacy delegation guidance and shared staffing independently of the bounded chat coordinator", () => {
    const prompt = chiefOfStaffSystemPrompt("chief", bots, true, "TRUSTED SOFTBOTS STATUS\nfreshness=fresh");
