// `squadbots serve --domain maus.example.com`: HTTPS on your own domain
...
export async function ensureCaddy(options: {
  ...
  const scratch = mkdtempSync(join(tmpdir(), "squadbots-caddy-"));
  try {
...
export function caddyfileFor(input: { domain: string; appPort: number; webhookPort: number }): string {
  return [
    "# Written by squadbots serve --domain. Edit the command, not this file.",
    "{",
    "\tadmin off",
    "\tlog {",
    "\t\tlevel WARN",
