// `softbots serve --domain maus.example.com`: HTTPS on your own domain
...
export async function ensureCaddy(options: {
  ...
  const scratch = mkdtempSync(join(tmpdir(), "softbots-caddy-"));
  try {
...
export function caddyfileFor(input: { domain: string; appPort: number; webhookPort: number }): string {
  return [
    "# Written by softbots serve --domain. Edit the command, not this file.",
    "{",
    "\tadmin off",
    "\tlog {",
    "\t\tlevel WARN",
