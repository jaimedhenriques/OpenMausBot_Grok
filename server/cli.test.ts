...
describe("squadbots command line", () => {
  ...
  describe("the two links one pairing window has", () => {
    ...
    const invite = `squadbots://pair?address=https%3A%2F%2Fmini.example&token=omb_pair_${"a".repeat(43)}&name=mini`;
    ...
    it("gives an Android phone the app-scheme QR, because its scanner rejects https", () => {
      ...
      expect(out).toContain("Scan that in the Squadbots app");
    ...
    it("says plainly when an Android phone asked for an app link this server cannot build", () => {
      ...
      expect(out).not.toContain("Scan that in the Squadbots app");
    });
  });
  ...
  it("serve: starts the server, prints the pairing link, and stops on SIGTERM", async () => {
    ...
    const child = spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "squadbots.ts"), "serve", "--port", String(port), "--data-dir", join(home, "data"), "--label", "cli test", "--public-url", "https://mini.example"], {
      cwd: join(SERVER_DIR, ".."),
    ...
    try {
      ...
      expect(out).toContain(`Squadbots is running on http://127.0.0.1:${port}, reachable at https://mini.example`);
      expect(out).toMatch(/pairing code:  [A-Z2-9]{4}-[A-Z2-9]{4}-[A-Z2-9]{4}/);
      expect(out).toContain("open or scan:  https://mini.example/pair#code=");
      expect(out).toMatch(/[▀▄█]/);
      const descriptor: any = await (await fetch(`http://127.0.0.1:${port}/.well-known/squadbots/environment`)).json();
...
describe("terminal onboarding commands", () => {
  ...
  it("runs explicit setup once and explains how to start without launching a server", async () => {
    ...
    expect(output.log).toHaveBeenCalledWith(expect.stringContaining("Start with: squadbots"));
...
describe.skipIf(process.platform === "win32")("serve --tunnel", () => {
  const cli = (args: string[], env: NodeJS.ProcessEnv) =>
    spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "squadbots.ts"), ...args], {
      cwd: join(SERVER_DIR, ".."),
  ...
  it("refuses without an account and says what to do; no local-only fallback", async () => {
    ...
    try {
      ...
      expect(err).toContain("run `squadbots login` first");
      let dead = false;
      try {
        await fetch(`http://127.0.0.1:${port}/api/health`);
      } catch {
  ...
  it("a fleet credential in the environment serves --tunnel with no account file and no code", async () => {
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("Squadbots is running") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      ...
      while (Date.now() < gatewayDeadline && status !== 200) {
        try {
          status = (await fetch(`${gateway}/.well-known/squadbots/environment`)).status;
        } catch {
          status = 0;
        }
  ...
  it("serves at the account's public address through the gateway, where every request is remote", async () => {
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("open or scan:") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      expect(out).toContain(`Squadbots is running on http://127.0.0.1:${port}, reachable at ${stub.endpointUrl}`);
      ...
      while (Date.now() < gatewayDeadline) {
        try {
          descriptor = await fetch(`${gateway}/.well-known/squadbots/environment`);
...
describe.skipIf(process.platform === "win32")("serve --domain", () => {
  it("runs a managed Caddy for the domain and serves the pairing link there", async () => {
    ...
    const port = 21000 + Math.floor(Math.random() * 9000);
    const child = spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "squadbots.ts"), "serve", "--domain", "omb.example.test", "--port", String(port), "--data-dir", dataDir], {
      cwd: join(SERVER_DIR, ".."),
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("open or scan:") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      expect(out).toContain(`Squadbots is running on http://127.0.0.1:${port}, reachable at https://omb.example.test`);
