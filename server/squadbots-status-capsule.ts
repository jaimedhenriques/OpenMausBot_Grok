...
type SquadbotsSlot = z.output<typeof slotSchema>;
type SquadbotsUi = z.output<typeof uiSchema>;
type SquadbotsCapsule = z.output<typeof capsuleSchema>;

export interface SquadbotsStatusDigest {
...
export interface SquadbotsStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): SquadbotsUi {
  return {
    two_up: twoUp,
...
function sameUi(left: SquadbotsUi, right: SquadbotsUi): boolean {
  return (
...
function unsignedDocument(capsule: SquadbotsCapsule): JsonObject {
...
function validSlotState(slot: SquadbotsSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): SquadbotsCapsule | null {
...
function unknownDigest(
  reason: NonNullable<SquadbotsStatusDigest["reason"]>,
  freshness: SquadbotsStatusDigest["freshness"] = "unknown",
  receipt?: Pick<SquadbotsStatusDigest, "observedAt" | "receiptSha256">,
): SquadbotsStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
    reason,
...
export function readSquadbotsStatus(
  ...
  const result: SquadbotsStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function squadbotsStatusSystemPrompt(options: SquadbotsStatusReadOptions = {}): string {
  const status = readSquadbotsStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED SQUADBOTS STATUS (read-only, validated, no transcript or credential data):",
...
/** @deprecated Squadbots rebrand — use squadbotsStatusSystemPrompt */
export const openMausStatusSystemPrompt = squadbotsStatusSystemPrompt;
