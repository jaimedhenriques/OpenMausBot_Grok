// `softbots` on the command line: run the server anywhere and pair devices
// to it. One implementation for three homes — `npx softbots` (the npm
...
//
//   softbots setup [--data-dir ~/.softbots]
//   softbots start [serve options]
//   softbots serve [--port 8799] [--data-dir ~/.softbots] [--label "cab mini"]
//                     [--public-url https://host] [--tailscale | --tunnel | --domain HOST] [--no-pair]
//   softbots pair  [--label "My MacBook"] [--client] [--public-url https://host]
//   softbots sessions [revoke <id>]
//   softbots status
//   softbots login [--email you@example.com]
//   softbots logout
//
...
//
// This module only exports; softbots.ts is the entry that runs main(), so
...
export const USAGE = `softbots — your team of AI bots, ready in a few steps

  softbots                         set up once, then open your workspace
  softbots setup [--data-dir DIR]
  softbots start [the same options as serve]
  softbots serve [--port 8799] [--data-dir DIR] [--label NAME]
                    [--public-url https://host] [--tailscale | --tunnel | --domain HOST] [--no-pair]
  softbots pair  [--label NAME] [--client] [--phone ios|android]
                    [--public-url https://host]
  softbots sessions [revoke ID]
  softbots status
  softbots login [--email you@example.com]
  softbots logout
  softbots access list | add EMAIL [--chat-only] | remove EMAIL
  softbots service install [--domain HOST | --tunnel | --tailscale] [--port N] [--data-dir DIR] | uninstall
  softbots browser install [--with-deps] | status
  softbots fleet init --domain HOST [--operator USER] | create NAME --admin EMAIL [--member EMAIL] [--brand FILE]
...
start   same as softbots: use your saved settings and open the workspace
...
login   signs this machine in to an Softbots account (an emailed code)
...
service keep the server running across reboots: writes a systemd unit
        ...
        permanently first (npm install -g softbots).
...
--tunnel     serve at a public https://….softbots.com address through a
             ...
             \`softbots login\` once on this machine first.
...
Install once with \`npm install -g softbots\`, then type \`softbots\`.
Or run without a global install: \`npx softbots\`. Node 24+ is required.
`;
...
export async function isWorkspaceRunning(options: CliOptions): Promise<boolean> {
  try {
    ...
    const descriptor = await api(options.port, "/.well-known/softbots/environment");
...
export async function verifyPhoneEndpoint(port: number, origin: string): Promise<boolean> {
  ...
  try {
    const local = await api(port, "/.well-known/softbots/environment");
    const remote = await fetch(`${origin}/.well-known/softbots/environment`, { signal: AbortSignal.timeout(5000), redirect: "error" });
...
export function applyStartupPreferences(options: CliOptions, saved: AppConfig["cliStartup"]): CliOptions {
  ...
  if (saved.access === "public-url" && (!saved.publicUrl || !normalizePhoneOrigin(saved.publicUrl))) {
    throw new Error("The saved phone address is not a valid HTTPS origin. Run softbots setup to correct it, or softbots --local to start only on this computer.");
  }
  return {
    ...options,
...
async function showPhonePairing(options: CliOptions, origin: string | undefined, log: (line: string) => void): Promise<boolean> {
  ...
  if (!ready) {
    ...
    log("Check the HTTPS connection, then run softbots pair again with the same --data-dir and --port.");
    return false;
  }
 ...
 * the iOS app read. `inviteUrl` is the softbots:// scheme the native
...
export function pairingBlock(input: {
  ...
  if (target) {
    ...
    lines.push("");
    if (scanInvite) {
      lines.push(`Scan that in the Softbots app. For a browser instead, open the web`);
      ...
      lines.push(`above into the Softbots app.`);
    }
  }
  return lines.join("\n");
}
...
async function mintPairing(port: number, options: { label?: string; client?: boolean; publicUrl?: string; phone?: "ios" | "android" }): Promise<string> {
  ...
  const invite = typeof body.credential === "string" && address
    ? `softbots://pair?address=${encodeURIComponent(address)}&token=${encodeURIComponent(body.credential)}${typeof body.serverName === "string" ? `&name=${encodeURIComponent(body.serverName)}` : ""}`
...
export async function runPair(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Softbots server on http://127.0.0.1:${options.port}; start one with \`softbots serve\` or set OMB_PORT`);
    return 1;
  }
  if (process.stdin.isTTY && process.stdout.isTTY && !options.label && !options.client) {
    ...
    if (!origin || !normalizePhoneOrigin(origin)) {
      ...
      console.log("Stop the server, run softbots setup and choose phone access, then start softbots again.");
      return 1;
    }
...
export async function runSessions(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no Softbots server on http://127.0.0.1:${options.port}`);
    return 1;
  }
  if (options.revoke) {
  ...
  if (!sessions.length) {
    console.log("no paired devices yet: run `softbots pair`");
    return 0;
  }
...
export function formatSessions(sessions: Array<{ id: string; label: string; scopes: string[]; lastSeenAt: number; expiresAt: number }>, now = Date.now()): string {
  ...
  return [line(head), ...rows.map(line), "", "revoke one with: softbots sessions revoke <id>"].join("\n");
}

export async function runStatus(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  let code = 0;
  try {
    const res = await fetch(`http://127.0.0.1:${options.port}/.well-known/softbots/environment`);
    const body: any = await res.json();
    io.log(options.json ? JSON.stringify(body, null, 2) : `${body.label} · Softbots ${body.version} on ${body.platform} · id ${body.environmentId}`);
  } catch {
    io.error(`no Softbots server on http://127.0.0.1:${options.port}`);
    code = 1;
  }
  if (!options.json) {
...
export async function runAccess(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (options.accessAction === "list") {
    if (!admins.length && !members.length) {
      io.log("nobody can sign in with an email yet; pairing codes only. Add someone with: softbots access add you@example.com");
      return 0;
    }
  ...
  if (options.accessAction === "remove") {
    ...
    io.log(`${entry} can no longer sign in (existing sessions stay until they expire or are revoked with \`softbots sessions revoke\`)`);
    return 0;
  }
...
export async function runLogin(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  const email = (options.email ?? (await io.ask("Email for your Softbots account: "))).trim();
  if (!email) {
    io.error("an email address is needed: softbots login --email you@example.com");
    return 1;
  }
  try {
  ...
  io.log("Serve there with:  softbots serve --tunnel");
  return 0;
}
...
export async function runBrowser(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (options.browserAction === "status") {
    ...
    if (status.kind !== "ready" && status.installable) io.log("install it with:  softbots browser install");
  ...
  try {
    ...
    if (process.platform === "linux" && !options.withDeps) io.error("on Linux, install Chrome's system libraries with `sudo softbots browser install --with-deps`, then retry `softbots browser install` as the user running serve");
    return 1;
  }
  ...
  if (process.platform === "linux" && options.withDeps) io.log("if serve runs as another user, run `softbots browser install` from that user's login shell too");
  return 0;
}
...
async function planTunnel(options: CliOptions, log: (line: string) => void): Promise<TunnelPlan | { error: string }> {
  ...
  if (credential) {
    ...
    if (!describeTunnelAccount(account.credentials.read()).email) {
      return { error: "no account on this machine yet: run `softbots login` first, then `softbots serve --tunnel`" };
    }
    ...
    if (!access) return { error: "this machine has no public address; run `softbots login` again" };
  }
  let binary: string;
  try {
...
export async function runServe(options: CliOptions, log: (line: string) => void = console.log): Promise<number> {
  ...
  if (await serverUp(options.port)) {
    console.error(`something already answers on http://127.0.0.1:${options.port}; use \`softbots pair\` against it, or --port for a second server`);
    return 1;
  }
  ...
  try {
    ...
    if (exited !== null) {
      if (exited !== 0) log(`Softbots could not start.${logPath ? ` Details: ${logPath}` : " See the output above."}`);
      return exited;
    }
    if (stopping) return await childExit;
    if (!(await serverUp(options.port, child.pid))) {
      console.error(`Softbots did not become ready within a minute.${logPath ? ` Details: ${logPath}` : " See its output above."}`);
      await stop();
      return 1;
    }
    ...
    log("");
    log(`Softbots is running on http://127.0.0.1:${options.port}${publicUrl ? `, reachable at ${publicUrl}` : ""}`);
    if (options.guided) {
    ...
    if (options.pair && options.phone) {
      ...
      log("");
      log("another device later:  softbots pair --label \"Kitchen iPad\"");
    }
    log(options.guided ? "\nKeep this terminal open while using your bots. Ctrl+C stops the server, not your saved work." : "stop with Ctrl+C");
    if (options.guided) log("Next time: softbots · Change AI or phone setup: softbots setup · Pair another phone: softbots pair");
...
export async function runOnboardingCommand(
  ...
  if (options.command === "setup" && !interactive) {
    io.error("Setup needs an interactive terminal. Run `npx softbots setup` in a terminal, then use `npx softbots serve` for unattended starts.");
    return 1;
  }
  ...
  try {
    if (options.command === "setup" || !(await isSetupComplete(options.dataDir))) {
      if (!interactive) {
        io.error("No completed setup was found. Run `npx softbots setup` in an interactive terminal first, or use `npx softbots serve` with an existing configuration.");
        return 1;
      }
      if (!(await runSetup({ dataDir: options.dataDir, port: options.port }))) {
        io.log("Setup cancelled. Run softbots when you're ready.");
        return 130;
      }
    }
    ...
    if (options.command === "setup") {
      io.log("\nAll set. Start with: softbots (or npx softbots without a global install).");
    ...
    if (!(error instanceof SetupCancelled)) throw error;
    io.log("\nSetup stopped. Any AI setup already saved is kept; no server was started. Run softbots setup to continue.");
    return 130;
  }
}
