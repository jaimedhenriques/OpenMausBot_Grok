...
// into first-class Softbots chat cards. The agent never authors an auth
...
function initializeResult(id: unknown, protocolVersion: unknown): Json {
  return {
    ...
    result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "softbots-connectors", version: "1" },
    },
  };
}
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `Softbots showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "Softbots is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
