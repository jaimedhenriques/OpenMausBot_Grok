 ...
 * is there to forward traffic from elsewhere (`softbots serve --tunnel`),
...
const DESKTOP_OWNER_HEADER = "x-softbots-desktop-owner";
...
export function resolveRequestAuth(req: IncomingMessage, options: ResolveOptions): RequestAuthResult {
  ...
  if (loopback) {
    const companionToken = headerValue(req.headers["x-softbots-companion-auth"]);
    if (companionToken && options.loopbackMutationToken !== undefined) {
      if (
        !secureTokenMatch(companionToken, options.companionMutationToken ?? "") ||
        req.headers["x-softbots-companion"] !== "1" ||
        !/^[\w-]{1,128}$/.test(headerValue(req.headers["x-softbots-companion-device"]) ?? "") ||
