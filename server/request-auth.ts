 ...
 * is there to forward traffic from elsewhere (`squadbots serve --tunnel`),
...
const DESKTOP_OWNER_HEADER = "x-squadbots-desktop-owner";
...
export function resolveRequestAuth(req: IncomingMessage, options: ResolveOptions): RequestAuthResult {
  ...
  if (loopback) {
    const companionToken = headerValue(req.headers["x-squadbots-companion-auth"]);
    if (companionToken && options.loopbackMutationToken !== undefined) {
      if (
        !secureTokenMatch(companionToken, options.companionMutationToken ?? "") ||
        req.headers["x-squadbots-companion"] !== "1" ||
        !/^[\w-]{1,128}$/.test(headerValue(req.headers["x-squadbots-companion-device"]) ?? "") ||
