...
beforeEach(() => {
  ...
  mocks.tunnelAccess.mockReturnValue({ endpoint: "https://fixture.softbots.test" });
...
describe("CLI startup lifecycle", () => {
  ...
  it("reopens a matching desktop workspace even when its server PID differs from the lease owner", async () => {
    ...
    writeFileSync(join(dataDir, "squadbots-server.lease"), JSON.stringify({
  ...
  it.each(["mismatched", "unreachable"])("never mints a phone code for a %s public endpoint", async (failure) => {
    ...
    expect(requests).toContain("https://fixture.example.test/.well-known/squadbots/environment");
  });
  ...
  it("offers Android client pairing only after verifying the same workspace, without claiming it is connected", async () => {
    ...
    // the squadbots:// one (android/core Connection.kt).
    const credential = `omb_pair_${"a".repeat(43)}`;
    const inviteUrl = `squadbots://pair?address=${encodeURIComponent(origin)}&token=${credential}&name=fixture`;
    let healthRequests = 0;
    const fetcher = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      expect(address).toMatch(/\/\.well-known\/squadbots\/environment$/);
      expect(init?.method).not.toBe("POST");
    ...
    expect(requests).toContain(`http://127.0.0.1:${options.port}/.well-known/squadbots/environment`);
    expect(requests).toContain(`${origin}/.well-known/squadbots/environment`);
    expect(requests.indexOf(`${origin}/.well-known/squadbots/environment`)).toBeLessThan(requests.indexOf(pairingRequest));
    ...
    expect(output).toContain("On Android, open the Squadbots app and scan the QR with its pairing scanner.");
