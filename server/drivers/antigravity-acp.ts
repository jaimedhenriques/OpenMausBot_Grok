...
export async function prepareAntigravityProfile(input: {
  ...
  if (/\r|\n|\0|%s/u.test(executable) || executable.includes(delimiter)) {
    throw new Error("The Softbots runtime path cannot safely suppress Antigravity browser launches.");
  }
      ...
      const timer = setTimeout(() => {
        ...
        if (method === "initialize") {
          ...
          reject(new Error(
            ...
            "Retry setup. If it still fails, share this error and your Softbots version; do not paste Google sign-in links or tokens.",
          ));
...
export async function validateAntigravityRuntime(runtime: AntigravityRuntime, expectedVersion: string): Promise<void> {
  const profileDirectory = await mkdtemp(join(tmpdir(), "softbots-antigravity-verify-"));
