...
// there as first-class pi tools (pi.registerTool). Squadbots ships this file
// and the pi driver spawns it — the pi repo itself is never touched.
//
// Protocol: the Squadbots proxies speak raw JSON-RPC 2.0 over stdio, one
 ...
 * Keeping it local avoids bundling Pi into Squadbots; the extension is
...
/** A minimal stdio JSON-RPC 2.0 MCP client, matched to Squadbots's
  ...
  async init(): Promise<void> {
    await this.call(
      "initialize",
      {
        ...
        capabilities: {},
        clientInfo: { name: "squadbots-pi", version: "1" },
      },
  ...
  const mounts = await Promise.all(
    serverEntries.map(async ([serverName, def]) => {
      ...
      try {
        ...
        process.stderr.write(`[squadbots-pi-mcp] ${serverName}: ${message}\n`);
        client?.dispose();
        return { serverName };
      }
    }),
  ...
  for (const mount of mounts) {
    ...
    for (const tool of tools) {
      if (!tool || typeof tool.name !== "string" || !tool.name.trim()) {
        process.stderr.write(`[squadbots-pi-mcp] ${serverName}: skipped a tool with no valid name\n`);
        continue;
      }
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[squadbots-pi-mcp] ${serverName}:${toolName}: ${message}\n`);
      }
    }
