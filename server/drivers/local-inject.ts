...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `SQUADBOTS_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.SQUADBOTS_PROBE_LOCAL_INJECT ?? process.env.SQUADBOTS_PROBE_LOCAL_INJECT;
