...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `SOFTBOTS_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.SOFTBOTS_PROBE_LOCAL_INJECT ?? process.env.SOFTBOTS_PROBE_LOCAL_INJECT;
