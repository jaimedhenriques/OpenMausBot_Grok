...
const EXCLUDES = `# Softbots checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "Softbots Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@softbots.local";
  env.GIT_COMMITTER_NAME = "Softbots Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@softbots.local";
  return env;
}
