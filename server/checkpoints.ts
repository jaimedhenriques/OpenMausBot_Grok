...
const EXCLUDES = `# Squadbots checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "Squadbots Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@squadbots.local";
  env.GIT_COMMITTER_NAME = "Squadbots Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@squadbots.local";
  return env;
}
