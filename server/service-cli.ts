// The `softbots service` command, kept separate from cli.ts so it can be
...
export function runServiceCommand(input: ServiceInstallInput, io: ServiceIo): number {
  ...
  if (!plan) {
    io.error("services are written for Linux (systemd) and macOS (launchd); on Windows, use Task Scheduler to run `softbots serve` at startup");
    return 1;
  }
  ...
  io.log(`logs: ${platform === "darwin" ? `${input.dataDir}/logs/service.log` : "journalctl -u softbots -f"}`);
  io.log(`change options later by running \`softbots service install\` again with the new ones, then: ${platform === "darwin" ? plan.activate[1] : "sudo systemctl daemon-reload && sudo systemctl restart softbots"}`);
  return 0;
}
