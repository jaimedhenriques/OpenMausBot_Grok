...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        ...
        let url = try XCTUnwrap(URL(string: "squadbots://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Milind%27s%20Mac&secretKey=\(secretKey)"))
    ...
    func testRejectsAPresentButInvalidSecureEntryKey() throws {
        ...
        let invalid = try XCTUnwrap(URL(string:
            "squadbots://pair?address=mac.local&token=\(token)&secretKey=not-a-p256-key"))
        XCTAssertNil(PairingInvite.parse(invalid))
    }

    func testPairingConsentShowsNormalizedOriginInsteadOfTrustingQRName() throws {
        let url = try XCTUnwrap(URL(string:
            "squadbots://pair?address=https%3A%2F%2FOTHER.Example%3A9443%2F" +
    ...
    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "squadbots://pair?address=mac.local&code=004209"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testLegacyTailnetInviteDropsUnselectedLocalFallbackKinds() throws {
        let url = try XCTUnwrap(URL(string:
            "squadbots://pair?address=macbook.tail1234.ts.net%3A8810&code=004209" +
            "&hosts=macbook.tail1234.ts.net,192.168.1.42,squadbots-aa.local"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testHostedTypedInviteCannotRetainDirectFallbacks() throws {
        ...
        let token = "omb_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string:
            "squadbots://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=\(encoded)"))
    ...
    func testRejectsMalformedOrDowngradedTypedEndpoints() throws {
        ...
        let invalidBase64 = try XCTUnwrap(URL(string:
            "squadbots://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=not-json"))
    ...
    func testSanitizesFallbackHostsAndKeepsOnlyTheConfirmedLocalOrigin() throws {
        ...
        let url = try XCTUnwrap(URL(string:
            "squadbots://pair?address=mac.local&code=004209&hosts=%20mac.local%20,other.local,,bad%2Fslash,has%20space"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["mac.local"])

        // and an invite with no usable candidate keeps the single address
        let empty = try XCTUnwrap(URL(string: "squadbots://pair?address=mac.local&code=004209&hosts=bad%2Fslash"))
    ...
    func testAcceptsSquadbotsAndLegacyOpenMausPairingSchemes() throws {
        for scheme in ["squadbots", "openmausbot"] {
            let url = try XCTUnwrap(URL(string: "\(scheme)://pair?address=mac.local&code=123456"))
            XCTAssertNotNil(PairingInvite.parse(url), "\(scheme) links must survive the product migration")
        }
    }
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "squadbots://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "squadbots://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "squadbots://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "squadbots://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "squadbots://pair?address=one.local&address=two.local&code=123456"))))
    }
