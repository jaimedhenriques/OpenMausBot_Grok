...
if (-not $args.Count -or $args[0] -in @('--help', '-h', 'help')) {
    ...
    Write-Output 'Options: OMB_PODMAN_MACHINE (default softbots), OMB_PODMAN_ENV_FILE (default .env)'
    exit 0
}
...
$machine = if ($env:OMB_PODMAN_MACHINE) { $env:OMB_PODMAN_MACHINE } else { 'softbots' }
